...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/combined_bg_m25n256.png}
...
\newday{2026-03-01} Worked on implementing some of Romeels feedback in the last meeting. First thing to do was turn $a$ in \cref{eq:equivalentDustTemp} into a passable argument to vary and explore different values of it to match the ebl data from simon driver in \cref{fig:driverSimulationComparison} recreating the same peak and shape. Also 'collecting' per galaxy dust temps so for every valid galaxy we now have $T_{eqv}$ and $z$ to plot the distribution to visualise the output temps from \cref{eq:equivalentDustTemp} so I can verify them, haven't check a specific paper but remember Romeel saying the mean temp should be somewhere between 20K-40K. Below is the first diagnostic plot for trying to find the ideal normalisation factor \footnote{i do have the ebl data now so i could use a monte carlo method to find the ideal value of $a$ that best matches the peak.}. 

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/combined_bg_m25n256_diagnostic.png}
...
\noindent In \cref{fig:m25n256CombinedTempNormalisationDiagnostic} I have plotted a range of different $a$ values to see which one works best but Driver ebl data needs to be overplotted to verify. However, the main problem I'm seeing is that the curve is not dipping down as much as we expect it to at all. In \cref{fig:driverSimulationComparison} the curve is almost at zero by $10^4$ but we notice that ours only really starts to dip at that point and does not seem to follow the same curve. Furthermore, looking at the dust histogram in the bottom left across all lightcone galaxies we see that a lower, negative $a$ gives us median dust temps much closer to expectations. Looking at the far IR peak from \cref{fig:driverSimulationComparison} and comparing to the values on the bottom right of \cref{fig:m25n256CombinedTempNormalisationDiagnostic}

Explanation for the temps may be that \cite{liangDustTemperaturesHighredshift2019} quotes the equation for only the specific $z$ range but we have applied it from 0 to 7 potentially introducing the discrepancy. Need to look into doing some more diagnostics and plotting $T_\mathrm{eqv}$ against some other parameters to see whats going on. 


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% RADIO EMISSION

Next step is working on radio emission part of the SED. Needed to do some literature review on how to actually approach do this- Romeel gave me a paper and a rough method to follow but need to iron that out. Outline approach in more detail below

For radio emission:
Radio emission in many galaxies is heavily dominated by the central supermassive black hole (SMBH), particularly in active galaxies, radio-loud quasars, and elliptical galaxies. There is two sources contributing to radio luminosity non-thermal synchrotron from SNe/SNR and thermal from free-free radition in HIII regions. \cite{thomasRadioGalaxyPopulation2021} models star formation radio emission using \cite{condonRadioEmissionNormal1992} we quote equations 10 and 11,
...
For far IR I still need to spend a little bit more time to figure out deciding on the normalisation factor and what gives best value. Need to add the overplotting of Simon Driver EBL data and maybe implementing an MCMC for calculating $a$ using his code. 
...
\begin{equation}
    \frac{P_\mathrm{Rad}}{10^{30}{\mathrm{erg}\textrm{s^{-1}}} = \frac{\dot{M_{BH}}}{4\times10^{17}\textrm{g s^{-1}}}
...
bin it by redshfit to see the contribution from each aspect and just nice plots to have in the report as well
