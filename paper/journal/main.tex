...
\newday{2026-01-27} The one thing is that the sSFR cut used for quenched galaxies is quite restrictive. The typical SFR of the universe (and galaxies at a given $M_*$) increases out to $z~2$. So having an sSFR cut that doesn't evolve commensurately is not ideal. One simple cut that has proved fairly effective is sSFR\_quenched(z) < 0.2 / t\_H(z), where t\_H is the age of the universe at redshift z. More apt definition is discussed in \cite{rodriguezmonteroMergersStarburstsQuenching2019}. I think the next thing is to combine these outputs into making a prediction for the observed cosmic SED. This means accounting for the fact that the higher-z ones will be dimmer. The easiest way is probably to make a light cone. Then can just sum up all the apparent magnitudes appropriately to get the total.
\vspace{1em} 
...
\setlength\itemsep{1em}
    \item I am not sure how to set up the symlink for accessing the data, so I just copied over the data for now, as I didn't want to mess something up on the Cuillin.
\end{itemize}

NEED TO REDO QUENCHED GALAXIES SED WITH LESS RESTRICTIVE BOUNDS AND LOOK AT A MOVING RESTRICTION THAT CHANGES AT REDSHIFT.

ssh -Y cuillin and clone repo in there and just work in there. symlink setup as 'sim'
...
bin by redshift and compare to andrews, driver paper

Have made the spectra for optical and low IR wavelengths the spectra is ready. got the lightcone generating code modified for my use case as described above and then some pretty simple code to run it for actually generating the lightcone. was run for the m25n256 run however need to rerun for m50n512 but it takes absoultely ages to run.
*ADD IMAGE HERE*

\newday{2026-01-29} Setting up the GitHub fully on the cuillin cluster to access larger data files from Simba run instead of having to copy data over. This involved reinstalling everything on my cuillin directory (miniconda, caesar, etc.) and then making the correct environment and dealing with fsps home environment variable. Found a way to access cuillin through vscode which makes editing and running code significantly easier (especially notebooks) however I am having issues with performance and it feels super slow and am spending more time troubleshooting making cuillin and vscode work than writing any actual code. 

Have not yet started any work on making the mid IR spectra.

\newday{2026-01-30} Updated galaxy classification function for sSFR cut to be less restrictive by commensurately updating it with age of universe at redshift $z$. It is now sSFR\_quenched($z$) < 0.2 / t\_H($z$), where t\_H is the age of the universe at redshift $z$ \cite{rodriguezmonteroMergersStarburstsQuenching2019}. In \cref{fig:updated_abs_mag_quenched_sed} and \cref{fig:updated_app_mag_quenched_sed} we see very similar overall shapes and trends, the main difference is just the number of galaxies observed at different redshifts.
...
Also have first major result of the project (although only for the smaller m25n256 run). In \cref{fig:optical_range_SED} we can see the flux from all galaxies in the optical part of the spectrum taken using a lightcone giving a rough idea of the optical 'colour' of the universe. 
...
\end{figure}
 ...
 In \cref{fig:galaxy_number_lightcone_distribuition} is also a distribution of the number of galaxies at different redshifts found by the lighcone function
