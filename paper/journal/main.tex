...
The paper discusses \daybib\cite{HillMasuiScott2018}.

\vspace{0.5em}

The article discusses \daybib\cite{Baker2021}.
...
Also assigned the first task, which is to replicate Figure 4 from \cite{HillMasuiScott2018}, but just for one snapshot of the SIMBA simulation. The goal is to essentially look at all galaxies at some redshift and sum up their fluxes for each band, then plot them by extracting wavelength and magnitudes. Adding up all intensities for each filter and then plotting each filter at its respective frequency.
