...
so have decided to first minimise the peak between Driver EBL data and my SED for the far IR regime. Was going to initially do this with \texttt{scipy.optimize.minimize} but that requires a continuous function which I don't have and adding in an interpolation step for between the data points is not great and Romeel said to try and avoid, its better to use the original discrete data. Currently what i have done is run a 'far IR sweep' where i've basically run the m25n256 version for a range of $a$ from -0.25 to 0.25 with 50 values and these will be tested. initially being done for m25n256 but needs to be redone for m100n1024. Also while making some preliminary plots there seems to be some volume based dependency on the plots when using different sims and they fit the EBL data slightly different. This is shown below
