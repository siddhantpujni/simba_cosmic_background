...
\newday*{2026-02-17}
Meeting with Romeel today to discuss the results above. Overall shape looks mostly reasonable but the peak seems to be a little low and so dust is slightly cooler than expected (should plot dust temperature to see what it looks like) also need to extend the far IR beyond $10^3$ to explore the dip downwards which is shown in \cref{fig:driverSimulationComparison}. Need to look into changing the leading parameter/normalisation factor 'a' in \cref{eq:equivalentDustTemp} up and down to see which one puts the peak at the right spot and this can be done easily manually just inspecting by eye to fit/match to the observed data we can get from Simon Driver's upcoming paper. \footnote{could look into applying the mcmc here with his data and our farIR peak.}

Finally, look into the radio part now which will just be a single point which we can find from equations 10, 11, and 12 from \cite{thomasRadioGalaxyPopulation2021}. This would be done by integrating from 5 solar masses up relative to total star formation and then multiply by a factor (not sure what factor).

Create stellar mass and redshift bins for the SED plots to present results in the paper.

note: \cref{eq:equivalentDustTemp} is technically only valid for a specific redshift range and we're taking it past that which is potentially why its colder than expected.
...
%powderday for pahs and summer for publish
